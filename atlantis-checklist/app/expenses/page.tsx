...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    ...
    if (sb) {
      ...
      const channel = sb.channel("expenses-realtime").on(
        "postgres_changes",
        { event: "*", schema: "public", table: "expenses" },
        (payload) => {
          if (payload.eventType === "INSERT") {
            const n = payload.new as Expense;
            setExpenses((prev) => prev.some((e) => e.id === n.id) ? prev : [n, ...prev]);
          } else if (payload.eventType === "DELETE") {
            const d = payload.old as { id: string };
            setExpenses((prev) => prev.filter((e) => e.id !== d.id));
          }
        }
      ).subscribe();
  ...
  useEffect(() => {
    if (!useDb && expenses.length > 0) saveLocal(expenses);
  }, [expenses, useDb]);
  ...
  const addExpense = useCallback(() => {
    ...
    const sb = getSupabase();
    if (sb) { sb.from("expenses").insert(expense).then(() => {}); }
  ...
  const resetForm = () => {
    setAmount(""); setDescription(""); setCategory("dining");
    setPaidBy("charly"); setSplit("50-50"); setPhotoUrl(null); setShowForm(false);
  };
  ...
  const settlement = expenses.reduce(
    (acc, e) => {
      if (e.split === "50-50") {
        ...
        if (e.paid_by === "charly") { acc.charlyPaid += e.amount; acc.ganksOwe += half; }
        else { acc.ganksPaid += e.amount; acc.charlyOwe += half; }
  ...
  return (
    <div className="min-h-screen font-body">
      <div className="sticky top-0 z-40 bg-gradient-to-b from-ocean-950 to-ocean-900 pb-4 shadow-lg shadow-ocean-950/50">
        ...
        <div className="mx-5 mt-3 bg-ocean-800/80 rounded-2xl p-4 border border-white/5">
          <div className="flex justify-between text-xs text-white/50 mb-2">
            ...
            <span className="text-white font-semibold text-sm">{formatMoney(settlement.total)}</span>
          </div>
          <div className="flex gap-3 text-xs mb-3">
            <div className="flex-1 bg-white/5 rounded-xl p-2.5 text-center">
              <div className="text-coral font-bold text-sm">{formatMoney(settlement.charlyPaid)}</div>
              <div className="text-white/40 mt-0.5">Charly paid</div>
            </div>
            <div className="flex-1 bg-white/5 rounded-xl p-2.5 text-center">
              <div className="text-gold font-bold text-sm">{formatMoney(settlement.ganksPaid)}</div>
              <div className="text-white/40 mt-0.5">Ganks paid</div>
            </div>
          </div>
          {expenses.length > 0 && (
            <div className={`text-center text-sm font-semibold py-2 rounded-xl ${
              netBalance > 0 ? "bg-gold/10 text-gold" : netBalance < 0 ? "bg-coral/10 text-coral" : "bg-mint/10 text-mint"
            }`}>
              {netBalance > 0 ? `Ganks owe Charly ${formatMoney(netBalance)}`
                : netBalance < 0 ? `Charly owes Ganks ${formatMoney(Math.abs(netBalance))}`
                : "All settled up!"}
            </div>
          )}
        </div>
      ...
      <div className="px-4 py-4 space-y-2">
        ...
        {expenses.map((e) => (
          <button key={e.id} onClick={() => setShowDetail(e)}
            ...
            {e.photo_url ? (
              <img src={e.photo_url} alt="" className="w-12 h-12 rounded-xl object-cover flex-shrink-0" />
            ) : (
            ...
            <div className="text-right flex-shrink-0">
              <div className="text-white font-semibold text-sm">{formatMoney(e.amount)}</div>
              <div className={`text-[10px] font-semibold mt-0.5 px-1.5 py-0.5 rounded-full inline-block ${
                e.paid_by === "charly" ? "bg-coral/15 text-coral" : "bg-gold/15 text-gold"}`}>
          ...
          <div className="bg-ocean-900 rounded-t-3xl w-full max-w-lg border-t border-white/10 max-h-[90vh] overflow-y-auto">
            <div className="p-5">
              <div className="flex justify-between items-center mb-5">
                ...
                <button onClick={resetForm} className="text-white/40 hover:text-white/70 p-1">
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)}
                    placeholder="0.00" className="w-full bg-white/10 text-white text-lg rounded-xl px-3 pl-8 py-3 placeholder-white/20 outline-none focus:ring-1 focus:ring-mint/50" />
                </div>
              </div>
              <div className="mb-4">
                <label className="text-white/50 text-xs font-medium block mb-1.5">What was it for?</label>
                <input type="text" value={description} onChange={(e) => setDescription(e.target.value)}
                  placeholder="Dinner at Nobu, taxi, etc." className="w-full bg-white/10 text-white text-sm rounded-xl px-3 py-3 placeholder-white/20 outline-none focus:ring-1 focus:ring-mint/50" />
              </div>
              <div className="mb-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CATEGORIES.map((c) => (
                    <button key={c.id} onClick={() => setCategory(c.id)}
                      className={`py-2 px-2 rounded-xl text-xs font-medium transition-all ${category === c.id ? "bg-white/15 text-white ring-1 ring-mint/40" : "bg-white/5 text-white/40 hover:bg-white/10"}`}>
                      {c.emoji} {c.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setPaidBy("charly")}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${paidBy === "charly" ? "bg-coral/20 text-coral ring-1 ring-coral/40" : "bg-white/5 text-white/40 hover:bg-white/10"}`}>Charly</button>
                  <button onClick={() => setPaidBy("ganks")}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${paidBy === "ganks" ? "bg-gold/20 text-gold ring-1 ring-gold/40" : "bg-white/5 text-white/40 hover:bg-white/10"}`}>Ganks</button>
                </div>
              </div>
              <div className="mb-4">
                <label className="text-white/50 text-xs font-medium block mb-1.5">How to split?</label>
                <div className="flex gap-2">
                  {[{ id: "50-50" as const, label: "50/50" }, { id: "charly-only" as const, label: "Charly only" }, { id: "ganks-only" as const, label: "Ganks only" }].map((s) => (
                    <button key={s.id} onClick={() => setSplit(s.id)}
                      className={`flex-1 py-2.5 rounded-xl text-xs font-semibold transition-all ${split === s.id ? "bg-mint/20 text-mint ring-1 ring-mint/40" : "bg-white/5 text-white/40 hover:bg-white/10"}`}>{s.label}</button>
                  ))}
                </div>
              </div>
              <div className="mb-6">
                ...
                {photoUrl ? (
                  <div className="relative">
                    ...
                    <button onClick={() => setPhotoUrl(null)} className="absolute top-2 right-2 bg-black/60 text-white rounded-full w-7 h-7 flex items-center justify-center">✕</button>
                  </div>
                ) : (
              ...
              <button onClick={addExpense} disabled={!amount || !description.trim()}
      ...
      {showDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-6" onClick={() => setShowDetail(null)}>
          <div className="bg-ocean-800 rounded-2xl p-5 max-w-sm w-full border border-white/10" onClick={(e) => e.stopPropagation()}>
            {showDetail.photo_url && <img src={showDetail.photo_url} alt="Receipt" className="w-full h-48 object-cover rounded-xl mb-4" />}
            <div className="text-center mb-4">
              <div className="text-3xl font-bold text-white">{formatMoney(showDetail.amount)}</div>
              <div className="text-white/70 text-sm mt-1">{showDetail.description}</div>
            </div>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-white/50"><span>Category</span><span className="text-white/80">{getCategoryEmoji(showDetail.category)} {CATEGORIES.find((c) => c.id === showDetail.category)?.label}</span></div>
              <div className="flex justify-between text-white/50"><span>Paid by</span><span className={showDetail.paid_by === "charly" ? "text-coral" : "text-gold"}>{showDetail.paid_by === "charly" ? "Charly" : "Ganks"}</span></div>
              <div className="flex justify-between text-white/50"><span>Split</span><span className="text-white/80">{showDetail.split === "50-50" ? "50/50" : showDetail.split === "charly-only" ? "Charly only" : "Ganks only"}</span></div>
              <div className="flex justify-between text-white/50"><span>Date</span><span className="text-white/80">{new Date(showDetail.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</span></div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowDetail(null)} className="flex-1 py-2.5 rounded-xl bg-white/10 text-white/70 text-sm font-medium hover:bg-white/15 transition-colors">Close</button>
              <button onClick={() => deleteExpense(showDetail.id)} className="py-2.5 px-4 rounded-xl bg-coral/10 text-coral text-sm font-medium hover:bg-coral/20 transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
